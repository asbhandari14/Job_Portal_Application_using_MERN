    ...
    for (const job of jobs) {
      try {
        const filteredUsers = await User.find({
          $or: [
            { "category.categoryFirst": job.category },
            { "category.categorySecond": job.category },
            { "category.categoryThird": job.category },
          ],
        });
        // console.log(filteredUsers)
        for (const user of filteredUsers) {
          const subject = `Hot Job Alert: ${job.title} in ${job.jobCategory} Available Now`;
